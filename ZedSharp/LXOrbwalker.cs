...
namespace ZedSharp {
    public class LXOrbwalker {
        ...
        private static readonly string[] Attacks = {
            ...
            "renektonsuperexecute", "rengarnewpassivebuffdash", "trundleq", "xenzhaothrust", "viktorqbuff",
        ...
        public static void AddToMenu(Menu menu) {
            ...
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_AARange", "AA Circle").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_AARange_Enemy", "AA Circle Enemy").SetValue(new Circle(true, Color.Pink)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_Holdzone", "Holdzone").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_MinionHPBar", "Minion HPBar").SetValue(new Circle(true, Color.Black)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_MinionHPBar_thickness", "^ HPBar Thickness").SetValue(new Slider(1, 1, 3)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_hitbox", "Show HitBoxes").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_Lasthit", "Minion Lasthit").SetValue(new Circle(true, Color.Lime)));
            menuDrawing.AddItem(
                new MenuItem("lxOrbwalker_Draw_nearKill", "Minion nearKill").SetValue(new Circle(true, Color.Gold)));
            ...
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_ExtraWindUp", "Extra Winduptime").SetValue(new Slider(80, 200, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AutoWindUp", "Autoset Windup").SetValue(false));
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_Priority_Unit", "Priority Unit").SetValue(
                    new StringList(new[] {"Minion", "Hero"})));
            menuMisc.AddItem(
                new MenuItem("lxOrbwalker_Misc_Humanizer", "Humanizer Delay").SetValue(new Slider(50, 100, 0)));
                    ...
                    Obj_AI_Minion minion in
                        from minion in
                            ObjectManager.Get<Obj_AI_Minion>().Where(
                                minion => minion.IsValidTarget() && InAutoAttackRange(minion))
                ...
                foreach (
                    Obj_AI_Turret turret in
                        ObjectManager.Get<Obj_AI_Turret>().Where(
                            turret => turret.IsValidTarget(GetAutoAttackRange(MyHero, turret))))
                    return turret;
            }
                    ...
                    Obj_AI_Minion minion in
                        ObjectManager.Get<Obj_AI_Minion>().Where(
                            minion =>
                                ...
                                minion.Team == GameObjectTeam.Neutral).Where(
                                    minion =>
        ...
        public static bool CanMove() {
            ...
            if (_lastAATick <= Environment.TickCount)
                return Environment.TickCount + Game.Ping/2 >= _lastAATick + MyHero.AttackCastDelay*1000 + extraWindup &&
                       _movement;
            return false;
        }
        ...
        private static Obj_AI_Base GetBestHeroTarget() {
            ...
            return hitsToKill < 4
                ? killableEnemy : SimpleTs.GetTarget(GetAutoAttackRange() + 100, SimpleTs.DamageType.Physical);
        }
        ...
        private static void CheckAutoWindUp() {
            ...
            if (windUp < 40)
                windUp = 40;
            Menu.Item("lxOrbwalker_Misc_ExtraWindUp").SetValue(windUp < 200
                ? new Slider(windUp, 200, 0) : new Slider(200, 200, 0));
        }
