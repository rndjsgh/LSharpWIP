    ...
    internal class Zed
    {
        ...
        public static void doLaneCombo(Obj_AI_Base target)
        {
            try
            {
                ...
                if (R.IsReady() && shadowR == null && dist < R.Range && canDoCombo(new[] { SpellSlot.Q, SpellSlot.W, SpellSlot.E, SpellSlot.R }))
                {
                    R.Cast(target);
                    return;
                }
                //eather casts 2 times or 0 get it to cast 1 time
                if (W.IsReady() && E.IsReady() && shadowW == null && !getWshad && shadowR != null && LastCastedSpell.LastCastPacketSent.Slot != SpellSlot.W)
                {
        ...
        public static void castItemsFull(Obj_AI_Base target)
        {
